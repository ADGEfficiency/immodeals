...
def remove_na(df, df_type='train', type_of_good='building', na_imputer_most_freq=None, na_imputer_median=None):
    ''' Returns a cleaned DataFrame
    ...
    if type_of_good == 'building':
        ...
        # List of columns where rows with missing values will be removed
        not_na_cols = ['surface_du_terrain', 'surface_habitable', 'revenu_cadastral']
        ...
        # List of columns where rows with missing values will be removed
        not_na_cols = ['revenu_cadastral', 'surface_habitable', 'salles_de_bains', 'toilettes']
    ...
    ### Remove NA values
    df_new = df_new.dropna(subset=not_na_cols)
...
def encoding_4_exploration(cleaned_df, type="train", type_of_good='building'):
    ...
    ## Label Encode etat_du_batiment
    good_state_classes = ['Excellent état', 'Fraîchement rénové', 'Bon', 'À rafraîchir', 'À rénover']
    ordered_classes = pd.api.types.CategoricalDtype(ordered = True ,categories = good_state_classes)
    cleaned_df['etat_du_batiment'] = cleaned_df['etat_du_batiment'].astype(ordered_classes)

    ## Label Encode classe_energetique
    energy_classes = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'not_given']
    ordered_en_classes = pd.api.types.CategoricalDtype(ordered = True ,categories = energy_classes)
    cleaned_df['classe_energetique'] = cleaned_df['classe_energetique'].astype(ordered_en_classes)
...
def pct_method(df, col, level):
    # Upper and lower limits by percentiles
    upper = np.percentile(df[col], 100 - level)
    lower = np.percentile(df[col], level)
...
def std_method(df, col):
    # Creating three standard deviations away boundaries
    std = np.std(df[col])
    upper_3std = np.mean(df[col]) + 3 * std
    lower_3std = np.mean(df[col]) - 3 * std
    # Returning the upper and lower limits
    return [lower_3std, upper_3std]


def outlier_definition(df, col, level=1, continuous=False, can_be_neg=False):
    # Obtaining the ranges
    pct_range = pct_method(df, col, level)
    iqr_range = iqr_method(df, col)
    std_range = std_method(df, col)
    if continuous is False:
        # Setting the lower limit fixed for discrete variables
        low_limit = np.min(df[col])
        ...
        if can_be_neg:
            ...
            low_limit = low_limit = np.max([pct_range[0],
                                            iqr_range[0],
                                            std_range[0]])
...
def limits_outliers(df, outlier_cols):
    # Dictionary to keep the (low_limit, high_limit)
    outliers_limits_dict = {}

    for col in outlier_cols:
        # Empirical test for continous feature or discrete one
        if df[col].nunique() > 100:
            outliers_limits_dict[col] = outlier_definition(df, col, level=1, continuous=True)
        else:
            outliers_limits_dict[col] = outlier_definition(df, col, level=1, continuous=False)

    return outliers_limits_dict


def remove_outliers(df, outlier_cols):
    outliers_limits_dict = limits_outliers(df, outlier_cols)
    df_no_outliers = df.copy()

    for feature, limits in outliers_limits_dict.items():
        # Make sure it's an int
        df_no_outliers[feature] = df_no_outliers[feature].astype('int')
        mask_low = df_no_outliers[feature] >= limits[0]
        mask_high = df_no_outliers[feature] <= limits[1]
        df_no_outliers = df_no_outliers[mask_low]
        df_no_outliers = df_no_outliers[mask_high]

    return outliers_limits_dict, df_no_outliers
